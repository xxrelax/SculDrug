...
class PocketLigandPairDatasetFeaturized(Dataset):
    ...
    def kmeans_plus_plus_init(self, data, k):
        ...
        indices = [torch.randint(0, n_points, (1,), device=device).item()]
        for _ in range(1, k):
            distances = torch.cdist(data[indices], data).min(dim=0)[0]
            prob = distances / distances.sum()
            next_index = torch.multinomial(prob, 1).item()
            indices.append(next_index)
        return data[indices]
    
    def create_virtual_nodes_per_batch(self, protein_pos, protein_atom_feature, protein_element, n_clusters=None, atoms_per_cluster=None):
        device = protein_pos.device
        virtual_atom_feature = torch.zeros_like(protein_atom_feature[0])  # 虚拟原子的特征初始化为零向量
        virtual_element = torch.tensor(-1, device=device)  # 虚拟原子的元素标识
        num_points = protein_pos.size(0)
        
        # 初始化存储结果的列表
        extended_protein_pos_list = [protein_pos]
        extended_protein_atom_feature_list = [protein_atom_feature]
        extended_protein_element_list = [protein_element]  # 新增元素列表
        virtual_mask_list = [torch.zeros(num_points, dtype=torch.bool, device=device)]
        group_indices_list = torch.full((num_points,), -1, dtype=torch.long, device=device)
        ...
        # 计算虚拟节点位置、特征和元素
        virtual_nodes_pos = []
        virtual_nodes_feature = []
        virtual_nodes_element = []
        virtual_nodes_group_indices = []

        for i in range(n_clusters):
            cluster_points = protein_pos[group_indices == i]
            if cluster_points.size(0) > 0:
                cluster_center = cluster_points.mean(dim=0)
                virtual_nodes_pos.append(cluster_center)
                virtual_nodes_feature.append(virtual_atom_feature)
        ...
        virtual_mask = torch.cat(virtual_mask_list, dim=0)

        return extended_protein_pos, extended_protein_atom_feature, extended_protein_element, virtual_mask, group_indices_list
