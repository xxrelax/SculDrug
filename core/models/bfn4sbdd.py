...
class BFN4SBDDScoreModel(BFNBase):
    ...
    def interdependency_modeling(
        ...
        output_pos = self.GNN(surface_pos, mu_pos_t,batch_surface, batch_ligand, time)
        pos_ligand_glo = mu_pos_t + output_pos

        real_protein_mask = ~virtual_mask  # 用于区分真实和虚拟原子的掩码
        real_protein_v = protein_v[real_protein_mask]  # 提取真实原子的特征
        h_protein = self.protein_atom_emb(protein_v)  # [N_protein, self.hidden_dim - 1]
        h_protein = self.protein_atom_emb(real_protein_v)  # 仅对真实原子进行嵌入

        # 处理虚拟原子
        h_protein_extended = torch.zeros((protein_v.size(0), self.hidden_dim - 1), device=protein_v.device)
        h_protein_extended[real_protein_mask] = h_protein  # 仅将真实原子的嵌入值赋回
        h_protein = h_protein_extended
        ...
        h_all, pos_all, batch_all, mask_ligand, virtual_mask, group_indices2 = compose_context_cross(
            h_protein=h_protein,
            h_ligand=init_ligand_h,
            pos_protein=protein_pos,
            pos_ligand=pos_ligand_glo,
            batch_protein=batch_protein,
            batch_ligand=batch_ligand,
            virtual_mask = virtual_mask,
            group_indices = group_indices_list
        )
        ...
        # time = 2 * time - 1
        outputs = self.unio2net(h_all, pos_all, mask_ligand, batch_all, virtual_mask, group_indices2,return_all=return_all, fix_x=fix_x)
    ...
    def reconstruction_loss_one_step(
        ...
        return self.loss_one_step(
            t, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, virtual_mask, group_indices_list,surface_pos, batch_surface
        )
