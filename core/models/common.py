...
def compose_context_cross(h_protein, h_ligand, pos_protein, pos_ligand, batch_protein, batch_ligand, virtual_mask, group_indices):
    ...
    virtual_mask_extended = torch.cat([
        virtual_mask,
        torch.zeros([batch_ligand.size(0)], dtype=torch.bool, device=virtual_mask.device)
    ], dim=0)

    group_indices_extended = torch.cat([
        group_indices,
        torch.full([batch_ligand.size(0)], -1, dtype=group_indices.dtype, device=group_indices.device)
    ], dim=0)

    virtual_mask_sorted = virtual_mask_extended[sort_idx]
    group_indices_sorted = group_indices_extended[sort_idx]
    return h_ctx, pos_ctx, batch_ctx, mask_ligand, virtual_mask_sorted, group_indices_sorted
...
class ConcatSquashLinear(nn.Module):
    ...
    def forward(self, ctx, x):
        gate = torch.sigmoid(self._hyper_gate(ctx))  # (N, dim_out)
        bias = self._hyper_bias(ctx)  # (N, dim_out)
        ret = self._layer(x) * gate + bias  # (N, dim_out)
        return ret