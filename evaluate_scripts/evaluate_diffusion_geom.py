...
from core.tools import eval_atom_type, scoring_func, analyze, eval_bond_length
...
# def find_sdf_files_two_levels_deep(base_path):
#     """
#     Find all .sdf files within the second-level subdirectories of the given base path.
#     Does not go deeper than two levels.

#     Args:
#         base_path (str): The directory to start searching from.

#     Returns:
#         list: A list of paths to the found .sdf files.
#     """
#     sdf_files = []
    
#     # First-level subdirectories
#     for first_level_dir in next(os.walk(base_path))[1]:  # Get only directories at first level
#         first_level_path = os.path.join(base_path, first_level_dir)
        
#         # Second-level subdirectories
#         for second_level_dir in next(os.walk(first_level_path))[1]:  # Get only directories at second level
#             second_level_path = os.path.join(first_level_path, second_level_dir)
            
#             # Find all .sdf files in this second-level directory
#             sdf_files.extend(glob(os.path.join(second_level_path, "*.sdf")))
...
if __name__ == '__main__':
    ...
    # Load generated data
    base_result_path = os.path.join(args.sample_path, "saved_data")
    results_fn_list = find_result_pt_files(base_result_path)
    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc='Eval')): 
        try:
            mol = Chem.SDMolSupplier(r_name)[0]
            if mol is None:
                continue
            ...
            for atom in mol.GetAtoms():
                atom_types.append(atom.GetSymbol())  # 获取原子类型
                pos = conformer.GetAtomPosition(atom.GetIdx())  # 获取原子坐标
                pred_pos.append([pos.x, pos.y, pos.z])  # 存储为 [x, y, z]
            if len(pred_pos) == 0:
                continue
            pred_atom_type = atom_types
            pred_atom_type = [analyze.atom_encoder[atom] for atom in pred_atom_type]
            ...
            logger.warning(f'File error at {r_name}: {e}')
            continue
        # 捕获重构错误
        except reconstruct.MolReconError as e:
            if args.verbose:
                logger.warning(f'Reconstruct failed at example {example_idx}: {e}')
            continue
        except KeyError as e:
            print(f"Missing atom: {e}")
            print(f"Available atom types: {list(analyze.atom_encoder.keys())}")
            raise

        try:
            mol = Chem.SDMolSupplier(os.path.splitext(r_name)[0] + '.sdf')[0]
            smiles = Chem.MolToSmiles(mol)
        except reconstruct.MolReconError:
            if args.verbose:
                logger.warning('Reconstruct failed %s' % f'{example_idx}')
            continue
        n_recon_success += 1  
        ...
        #             mol, data["ligand_filename"], protein_root=args.protein_root)
        ...
        #     n_eval_success += 1
            ...
            if args.verbose:
                logger.warning('Evaluation failed for %s' % f'{example_idx}')
            continue
    ...
    fraction_mol_stable = all_mol_stable / num_examples
    fraction_atm_stable = all_atom_stable / num_examples
    fraction_recon = n_recon_success / num_examples
    fraction_eval = n_eval_success / num_examples
    fraction_complete = n_complete / num_examples
    validity_dict = {
    ...
    atom_type_js,_ = eval_atom_type.eval_atom_type_distribution(success_atom_types)
    ...
    if args.save:
        eval_bond_length.plot_distance_hist(success_pair_length_profile,
                                            ...
                                            save_path=os.path.join("./test", f'pair_dist_hist_{args.eval_step}.png'))
    ...
    if args.save:
        torch.save({
            ...
            'all_results': results
        }, os.path.join("./test", f'metrics_{args.eval_step}.pt'))
